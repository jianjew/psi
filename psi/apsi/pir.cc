...
std::vector<uint8_t> ReadEcSecretKeyFile(const std::string &file_path) {
  ...
  std::vector<uint8_t> secret_key(::psi::kEccKeySize);
  ...
  *bucket_size = GetSizeFromStore(meta_info_store, kBucketSize);
  ...
  if (compress_flag == 1) {
    ...
    *compressed = false;
    ...
    YACL_THROW("error compress value:{}", compress_flag);
  }
...
PirResultReport PirServerOnline(
  ...
  std::vector<std::shared_ptr<ISenderDB>> sender_db(bucket_count);
  std::vector<std::shared_ptr<LabelPsiSender>> sender(bucket_count);
  ...
  for (size_t bucket_idx = 0; bucket_idx < bucket_count; ++bucket_idx) {
    ...
    sender_db[bucket_idx] = std::make_shared<SenderKvDB>(
        psi_params, oprf_key, bucket_setup_path, label_byte_count,
        nonce_byte_count, compressed);

    sender[bucket_idx] =
        std::make_shared<LabelPsiSender>(sender_db[bucket_idx]);
  }
  ...
  while (true) {
    ...
    for (size_t bucket_idx = 0; bucket_idx < bucket_count; bucket_idx++) {
      ...
      for (size_t i = 0; i < query_result.first.size(); ++i) {
        ...
        YACL_ENFORCE(result_ids.size() == key_columns.size());
        ...
        YACL_ENFORCE(result_labels.size() == label_columns_name.size(),
