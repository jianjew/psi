...
TEST_P(LabelPsiTest, Works) {
  ...
  if (params.use_kvdb) {
    sender_db = std::make_shared<SenderKvDB>(psi_params, oprf_key,
                                             kv_store_path, label_byte_count,
                                             nonce_byte_count, compressed);
  } else {
    sender_db = std::make_shared<SenderMemDB>(
  ...
  std::shared_ptr<IBatchProvider> batch_provider =
      std::make_shared<MemoryBatchProvider>(sender_keys, 5000, sender_labels);
