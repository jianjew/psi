...
class PirApiTest : public testing::TestWithParam<TestParams> {};

TEST_P(PirApiTest, Works) {
  SPDLOG_INFO("**********************************************************");
  SPDLOG_INFO("**********************************************************");
  SPDLOG_INFO("******************** pir api test begin ******************");
  SPDLOG_INFO("**********************************************************");
  SPDLOG_INFO("**********************************************************");
  auto params = GetParam();
  auto ctxs = yacl::link::test::SetupWorld(2);
  ...
  std::string client_csv_path ="examples/pir/data/client_api.csv";

  const std::string& server_csv_path = "{\n"
                             "  \"url\": \"http://172.16.16.1:5001/digits_active/data/query?pageSize=10\",\n"
                             "  \"datasource_kind\": 23\n"
                             "}";

  std::string pir_result_path = "examples/pir/data/pir_api_result.csv";
  ...
  std::vector<std::string> ids = {"id"};
  std::vector<std::string> labels = {"f11", "f12"};
  ...
  SPDLOG_WARN("###########pir_api_result_csv_data###########");
  ReadCsvFile(pir_result_path);
  ...
  SPDLOG_INFO("**********************************************************");
  SPDLOG_INFO("**********************************************************");
  SPDLOG_INFO("******************** pir api test end ********************");
  SPDLOG_INFO("****************data_count: {}, during_time: {}ms*********", params.ns, during_time);
  SPDLOG_INFO("**********************************************************");
  SPDLOG_INFO("**********************************************************");
}

INSTANTIATE_TEST_SUITE_P(Works_Instances, PirApiTest,testing::Values(TestParams{1, 10, 64}));
