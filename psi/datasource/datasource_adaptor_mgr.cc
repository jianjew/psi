...
#include <memory>
#include "odbc_adaptor_factory.h"
#include "tensor.h"
...
std::shared_ptr<DatasourceAdaptor> DatasourceAdaptorMgr::GetAdaptor(
  ...
  auto spec_pair = std::pair<std::string, DataSourceKind>(datasource_spec.connection_str, datasource_spec.kind);
  {
...
void DatasourceAdaptorMgr::RegisterBuiltinAdaptorFactories() {
  ...
  factory_maps_.insert({DataSourceKind::ODBC, odbc_adaptor_factory});
}
