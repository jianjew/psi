...
void KkrtPsiSender::PreProcess() {
  ...
  std::cout << "###[KkrtPsiSender]bucket_count: " << bucket_count_ << std::endl;

  if (bucket_count_ > 0) {
    std::vector<std::string> keys(config_.keys().begin(), config_.keys().end());
    std::cout << "###[KkrtPsiSender]recovery_manager_: " << recovery_manager_ << std::endl;
...
void KkrtPsiSender::Online() {
  ...
  size_t bucket_idx =
      recovery_manager_
          ...
          : 0;
  std::cout << "###KkrtPsiSender::Online,input_bucket_store_->BucketNum: "<< input_bucket_store_->BucketNum() << std::endl;
